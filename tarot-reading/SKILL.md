...
**3. Card Anchoring** (每张牌分两层，不可只讲其一)

每张牌必须包含：
- **画面层**：描述卡面上的视觉场景——人物在做什么、看向哪里、背景是什么
- **含义层**：基于该牌的核心关键词和标准含义，结合当前位置和问题语境，给出具体解读

两层缺一不可。不能只讲画面不给含义，也不能只列关键词不结合画面。
...
解读完成并收到用户反馈后，将本次显化解读的核心内容写入 `readings/manifestation.md`。
...
4. **显化进展** — 回顾 `readings/manifestation.md` 中的显化方案，更新进展，加入新的行动或洞见
